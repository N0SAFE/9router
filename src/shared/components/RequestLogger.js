            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              <tbody className="divide-y divide-border/50">
                {logs.map((log, i) => {
                  ...
                  const routing = parts.length > 7 ? parts[7] : "";
                  const isPending = status.includes("PENDING");
                  const isFailed = status.includes("FAILED");
                  const isSuccess = status.includes("OK");

                  const openDetails = () => {
                    const params = new URLSearchParams({ tab: "details", model, provider });
                  ...
                  return (
                    <tr
                      key={i}
                      onClick={openDetails}
                      className={`cursor-pointer hover:bg-surface-2/50 transition-colors ${isPending ? "bg-primary/5" : ""}`}
                    >
                      ...
                      <td className="px-3 py-1.5 border-r border-border">
                        <span className="px-1.5 py-0.5 rounded bg-surface-2 border border-border-subtle text-[10px] uppercase font-bold text-text-muted">
                          {provider}
                        </span>
                      </td>
                      <td className="px-3 py-1.5 border-r border-border truncate max-w-[150px] font-mono text-text-muted" title={parts[3]}>{parts[3]}</td>
                      <td className="px-3 py-1.5 border-r border-border text-right text-primary tabular-nums">{parts[4]}</td>
                      <td className="px-3 py-1.5 border-r border-border text-right text-success tabular-nums">{parts[5]}</td>
                      ...
                      <td className="px-3 py-1.5 font-mono text-[11px] text-text-muted max-w-[320px] truncate" title={routing || undefined}>
                        {routing || "—"}
                      </td>
                    </tr>
                  );
