 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, routing, apiKey, clientRawRequest, onRequestSuccess, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  const ctx = {
    provider, model, connectionId, routing,
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      saveUsageStats({ provider, model, tokens: usage, connectionId, routing, apiKey, endpoint: clientRawRequest?.endpoint, silent: true });
