...
- `services/` — `model.js`, `provider.js`, `accountFallback.js`, `accountPool.js` (provider account pool: error classification + session failure cache), `combo.js`, `compact.js`, `tokenRefresh/`+`tokenRefresh.js`, `oauthCredentialManager.js`, `usage/`, `projectId.js`, `kiroModels.js`/`qoderModels.js`.
